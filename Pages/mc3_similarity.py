...
with st.container():
    ...
    with col_select:
        select_box = st.selectbox(
            "Suspicious nodes", ["Option 1", "Option 2", "Option 3"])
        st.button('SHOW SIMILAR COMPANY')

    with col1:
        slider1 = st.slider("Company Size", min_value=0.0,
...
with line_col:
    st.markdown('<div style="height: 1000px; width: 1px; background-color: #d3d3d3;"></div>',
